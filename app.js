  ...
  try {
    ...
    uzenetek.forEach((uzenet) => {
      ...
      const profilkepHTML = uzenet.profilkep 
        ? `<img src="${uzenet.profilkep}" style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover; margin-right: 15px; vertical-align: top;">`
      ...
      let mediaHTML = '';
      if (uzenet.media) {
        if (uzenet.mediaType === 'image') {
          mediaHTML = `<img src="${uzenet.media}" class="uzenet-img" alt="Kép" style="max-width: 100%; max-height: 300px; border-radius: 8px; margin: 10px 0;">`;
        } else if (uzenet.mediaType === 'video') {
          mediaHTML = `<video class="uzenet-video" controls style="max-width: 100%; max-height: 300px; border-radius: 8px; margin: 10px 0;"><source src="${uzenet.media}" type="video/mp4"></video>`;
        } else if (uzenet.mediaType === 'gif') {
          mediaHTML = `<img src="${uzenet.media}" class="uzenet-img" alt="GIF" style="max-width: 100%; max-height: 300px; border-radius: 8px; margin: 10px 0;">`;
        }
      }
        ...
        (function() {
          ...
          if (userData) {
            container.innerHTML = \`
              ...
              <form action="/uj-uzenet" method="POST" enctype="multipart/form-data" style="margin-top: 20px;">
                ...
                <textarea name="uzenet" required placeholder="Írd ide az üzeneted..." style="width: 100%; padding: 15px; font-size: 16px; border: 2px solid #667eea; border-radius: 10px; margin-bottom: 10px; font-family: inherit; resize: vertical; min-height: 80px;"></textarea>
                <div style="margin: 10px 0;">
                  <label style="color: #667eea; font-weight: bold; display: block; margin-bottom: 8px;">📎 Média feltöltése (opcionális):</label>
                  <input type="file" name="media" accept="image/png,image/jpeg,image/gif,video/mp4,video/webm" style="padding: 10px; border: 2px solid #667eea; border-radius: 8px; width: 100%; cursor: pointer;">
                  <small style="color: #999;">Támogatott: JPG, PNG, GIF, MP4, WebM (max 10 MB)</small>
                </div>
    ...
    res.send(getStyle() + getMenu() + getChatbotWidget() + '<div class="container"><h1>❌ Hiba!</h1><p><strong>Részletek:</strong> ' + error.message + '</p></div>');
  }
});
...
app.post('/uj-uzenet', async (req, res) => {
  try {
    ...
    if (req.files && req.files.media) {
      ...
      ujUzenet.media = 'data:' + mediaType + ';base64,' + base64Data;
      
      if (mediaType.startsWith('image/')) {
        ujUzenet.mediaType = mediaType === 'image/gif' ? 'gif' : 'image';
      } else if (mediaType.startsWith('video/')) {
        ujUzenet.mediaType = 'video';
      }
    }
