...
MongoClient.connect(mongoUrl)
  .then(client => {
    ...
    schedule.scheduleJob('0 0 * * *', async () => {
      try {
        const result = await uzenetekCollection.deleteMany({});
        console.log(`✅ ${result.deletedCount} üzenet törölve az üzenőfalról!`);
      } catch (error) {
        console.error('❌ Hiba az üzenetek törlése során:', error);
      }
    });
    
    console.log('📅 Üzenettörlési schedule aktiválva! Minden nap éjfélkor törlődnek az üzenetek.');
  })
  .catch(error => {
